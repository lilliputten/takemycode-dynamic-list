import { resetCheckedApiUrl } from '@/config/routes';
import { jsonContentType } from '@/config/server';
import { APIError } from '@/shared/errors/APIError';
import { TServerDetailsResponse } from '@/types/server';

export async function resetCheckedOnServer() {
  const url = resetCheckedApiUrl;
  const method = 'POST';
  const headers = {
  ...
  try {
    ...
    if (!ok || status !== 200) {
      ...
      console.error('[api/methods/resetCheckedOnServer] response error', errMsg, {
        ok,
        data,
        statusText,
        status,
    ...
    // eslint-disable-next-line no-console
    console.error('[api/methods/resetCheckedOnServer] caught error', {
      error,
      url,
    });
    // debugger; // eslint-disable-line no-debugger
