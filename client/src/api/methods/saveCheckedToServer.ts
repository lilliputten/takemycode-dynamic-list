import { saveCheckedApiUrl } from '@/config/routes';
...
export async function saveCheckedToServer(params: TParams) {
  const url = saveCheckedApiUrl;
  const method = 'POST';
  const headers = {
  ...
  try {
    ...
    if (!ok || status !== 200) {
      ...
      console.error('[api/methods/saveCheckedToServer] response error', errMsg, {
        ok,
        data,
        statusText,
        status,
    ...
    // eslint-disable-next-line no-console
    console.error('[api/methods/saveCheckedToServer] caught error', {
      error,
      url,
    });
    // debugger; // eslint-disable-line no-debugger
